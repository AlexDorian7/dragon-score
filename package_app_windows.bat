...
xcopy /S /Q target\LogiQuest\libs\* target\installer\input\libs\
...
"%JAVA_HOME%\bin\jdeps" ^
  ...
  --print-module-deps target\classes\team\logica_populi\dragonscore\Main.class > temp.txt
...
call "%JAVA_HOME%\bin\jpackage" ^
  ...
  --name LogiQuest ^
  --main-class team.logica_populi.dragonscore.JarLauncher ^
  --main-jar %MAIN_JAR% ^
  --java-options -Xmx2048m ^
  --runtime-image target/java-runtime ^
  --app-version %APP_VERSION% ^
  --vendor "Logica Populi" ^
...
rem  --icon src/main/logo/windows/duke.ico ^